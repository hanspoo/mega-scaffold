...
import { ServicioPedidosPrisma } from './ServicioPedidosPrisma';
...
describe('crear pedido directo en prisma', () => {
...
describe('crear pedidos', () => {
  it('crea con mismo email', async () => {
    ...
    expect(pedido.name).toBe(contact.name);
  });
  it.skip('crea todos los items', async () => {
    const servicio = new ServicioPedidosPrisma(cart, contact);
    const pedido = await servicio.crearPedido();
    expect(pedido).toBe(contact.name);
  });
});
